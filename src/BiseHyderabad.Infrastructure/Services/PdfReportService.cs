...
public class PdfReportService : IPdfReportService
{
    ...
    private static Image? GetLogoImage(float width, float height)
    {
        string[] candidatePaths = [
            ...
            System.IO.Path.Combine(Directory.GetCurrentDirectory(), "src", "BiseHyderabad.Web", "wwwroot", "images", "bise-logo.png")
        ];
    ...
    public async Task<byte[]> GenerateEnrollmentCardPdfAsync(int enrollmentId)
    {
        ...
        using (var pdf = new PdfDocument(writer))
        {
            ...
            titleHeader.Add(new Paragraph("BOARD OF INTERMEDIATE & SECONDARY EDUCATION HYDERABAD")
            ...
            titleHeader.Add(new Paragraph($"Academic Session: {student.AcademicYear?.YearName ?? "2025–2026"} | BISE Hyderabad Sindh")
            ...
            var sig3 = new Cell().SetBorder(Border.NO_BORDER).SetTextAlignment(TextAlignment.CENTER)
                .Add(new Paragraph("___________________________\nSecretary BISE Hyderabad").SetFont(fontBold).SetFontSize(7f).SetFontColor(PrimaryBlue));
    ...
    public async Task<byte[]> GenerateRollNumberSlipPdfAsync(int examFormId)
    {
        ...
        using (var pdf = new PdfDocument(writer))
        {
            ...
            textCell.Add(new Paragraph("BOARD OF INTERMEDIATE & SECONDARY EDUCATION HYDERABAD")
    ...
    public async Task<byte[]> GenerateCertificatePdfAsync(int certificateIdOrResultId)
    {
        ...
        using (var pdf = new PdfDocument(writer))
        {
            ...
            certCell.Add(new Paragraph("BOARD OF INTERMEDIATE & SECONDARY EDUCATION HYDERABAD")
            ...
            var qrBytes = _certService.GenerateQrCodePng($"https://bisehyderabad.edu.pk/certificate/verify/{cert.CertificateNumber}");
            ...
            ctrlCell.Add(new Paragraph("___________________________\nController of Examinations\nBISE Hyderabad").SetFont(fontBold).SetFontSize(7.5f).SetFontColor(PrimaryBlue));
    ...
    public async Task<byte[]> GenerateBankChallanPdfAsync(int invoiceId)
    {
        ...
        using (var pdf = new PdfDocument(writer))
        {
            ...
            foreach (var copyName in copies)
            {
                ...
                rCell.Add(new Paragraph("BISE HYDERABAD COLLECTION A/C").SetFont(fontBold).SetFontSize(6.5f));
    ...
    public async Task<byte[]> GenerateCandidateListPdfAsync(int invoiceId)
    {
        ...
        using (var pdf = new PdfDocument(writer))
        {
            ...
            rCell.Add(new Paragraph("BOARD OF INTERMEDIATE & SECONDARY EDUCATION HYDERABAD")
