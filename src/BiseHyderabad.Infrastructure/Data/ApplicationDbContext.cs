...
public class ApplicationDbContext : DbContext
{
    ...
    public bool IsPlatformAdministrator => _tenantContext?.IsPlatformAdministrator == true;
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
            ...
            .Where(t => t.ClrType != typeof(Tenant) && t.FindProperty(nameof(User.TenantId)) != null))
        {
            ...
            var hasTenant = Expression.Property(currentTenantId, nameof(Nullable<int>.HasValue));
            var tenantMatches = Expression.Equal(tenantId, Expression.Property(currentTenantId, nameof(Nullable<int>.Value)));
            var platformBypass = Expression.Property(
                Expression.Constant(this),
                nameof(IsPlatformAdministrator));
            var filter = Expression.Lambda(
                Expression.OrElse(platformBypass, Expression.AndAlso(hasTenant, tenantMatches)),
                parameter);
